...
firebase.database().ref().child("Booked_Appointments").on('child_added', function (childSnapshot) {
	...
	var patientRef = database.ref('Patient_Details');
	var id = childSnapshot.key;

	childSnapshot.forEach(function (childSnapshot1) {
		
			var id2 = childSnapshot1.key;
			var date = childSnapshot1.child("Date").val();
			var doctor = childSnapshot1.child("Doctor_ID").val();
			var time = childSnapshot1.child("Time").val();			

			$(document).ready(function () {
		
		
				$('#tablehistory').append(
					'<tbody id="mytablehistory"><tr><td>' + doctor + '</td><td>' + id + '</td><td>' + date + '</td><td>' + time + '</td></tr></tbody>')
			});
	});
...
function update(id) {
	...
	$(document).ready(function () {
		...
		ref.once('value', function (data) {
			...
			var degree = data.child("Degree").val();
...
function setUpdate() {
	...
	if (btn == "Update Docter Details") {
		...
		firebase.database().ref().child("Doctor_Details").child(id).child("Degree").set(degree1);
		...
		firebase.database().ref().child("Doctor_Details").child(id).child("Fee").set(fee);
